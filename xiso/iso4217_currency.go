...
// ISOCurrency identifies a currency.
type ISOCurrency struct {
	Code string `json:"currencyCode"`
	Name string `json:"currencyName"`
}

// Currency validates and looks up currency with code which must be 3 letters.
func Currency(code string) (*ISOCurrency, error) {
	c := strings.ToUpper(code)
	if !reISO4217CurrencyCode.MatchString(c) {
		return nil, fmt.Errorf("invalid currency '%s' (must be 3 letters)", code)
	}
	name, have := iso4217Currency[c]
	if !have {
		return nil, fmt.Errorf("unknown currency '%s'", code)
	}
